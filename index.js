...
function getSession(userId) {
  if (!sessions.has(userId)) {
    sessions.set(userId, {
      ...
      qIndex: 0,
      currentQuestionPath: null, // e.g., 'categories.stylist'
      awaitingVideo: false,
    });
  }
...
const TEXT = {
  ...
  chooseCategory: {
    uz: 'Iltimos, yo\'nalishni tanlang:',
    ru: 'Пожалуйста, выберите направление:'
  },
  thanks: {
    uz: 'Rahmat! Endi savollarga javob bering.',
    ru: 'Спасибо! Теперь ответьте на вопросы.'
  },
  commonVideos: {
    ...
    uz: 'Tugadi! Arizangiz kanalga yuborildi. Javobni kuting. Rahmat! ✅',
    ru: 'Готово! Ваша анкета отправлена в канал. Ожидайте ответ. Спасибо! ✅'
  },
  reset: {
    uz: 'Sessiya yangilandi. /start buyrug\'ini bosing.',
    ru: 'Сессия сброшена. Нажмите /start.'
  },
  invalidPhone: {
    uz: 'Telefon raqam formati noto\'g\'ri. Masalan: +998901234567 yoki 90 123 45 67.',
    ru: 'Неверный формат номера. Например: +998901234567 или 90 123 45 67.'
  },
  promptReady: {
    uz: "Agar hammasini yuborgan bo'lsangiz, 'Tayyor' deb yozing.",
    ru: "Если всё отправили, напишите 'Готово'."
  }
};
...
const CATEGORIES = {
  uz: [
    'Stilist',
    'Kolorit (soch bo\'yash bo\'yicha mutaxassis)',
    'Vizajist (make-up artist)',
    'Manikyur/pedikyur ustasi',
    'Kiprikchi (lashmaker)',
    'Depilatsiya mutaxassisi',
    'Administrator'
  ],
  ru: [
    'Стилист',
...
// ======== QUESTIONS ========
// For brevity, keys map category -> array of questions (both languages)
const QUESTIONS = {
  uz: {
    'Stilist': [
    ...
    'Kolorit (soch bo\'yash bo\'yicha mutaxassis)': [
      ...
      'Sochni bo\'yashdan oldin qanday himoya vositalarini qo\'llasiz?',
  ...
  ru: {
    ...
    'Мастер маникюра/педикюра': [
      ...
      'Каким оборудованием стерилизуете? (стерилизатор, кварц, растворы)',
...
function categoryKeyboard(lang) {
  const rows = CATEGORIES[lang].map((c) => [Markup.button.callback(c, `cat_${c}`)]);
...
function formatSummary(lang, session, telegramUser) {
  const L = lang === 'ru' ? {
    title: 'Новая анкета',
    name: 'Имя', phone: 'Телефон', lang: 'Язык', cat: 'Категория',
    q: 'Ответы на вопросы',
    from: 'Отправитель',
  } : {
    title: 'Yangi anketa',
    name: 'Ism', phone: 'Telefon', lang: 'Til', cat: 'Kategoriya',
    q: 'Savollarga javoblar',
    from: 'Yuboruvchi',
  };

  const lines = [];
  lines.push(`📝 ${L.title}`);
  lines.push(`• ${L.name}: ${session.data.fullName || '-'}\n• ${L.phone}: ${session.data.phone || '-'}`);
  lines.push(`• ${L.lang}: ${LANGS[session.lang]}`);
  lines.push(`• ${L.cat}: ${session.data.category || '-'}`);
  lines.push(`\n${L.q}:`);
...
async function askFullName(ctx) {
  ...
  s.step = 'askFullName';
  await ctx.reply(TEXT.askFullName[s.lang || 'uz']);
}

async function askPhone(ctx) {
  const s = getSession(ctx.from.id);
  s.step = 'askPhone';
  await ctx.reply(TEXT.askPhone[s.lang || 'uz'], contactKeyboard(s.lang || 'uz'));
}

async function askLang(ctx) {
  const s = getSession(ctx.from.id);
  s.step = 'askLang';
  await ctx.reply(TEXT.askLang[s.lang || 'uz'], removeKb());
...
async function askCategory(ctx) {
  ...
  await ctx.reply(TEXT.chooseCategory[s.lang], categoryKeyboard(s.lang));
}

async function startQuestions(ctx) {
  const s = getSession(ctx.from.id);
  s.qIndex = 0;
  const qList = QUESTIONS[s.lang][s.data.category];
  s.currentQuestionPath = `${s.lang}:${s.data.category}`;
  s.step = 'askQuestions';
...
async function finalizeAndSend(ctx) {
  ...
  // Forward uploaded videos
  for (const mid of s.data.uploadedVideoIds) {
    try {
      await ctx.telegram.copyMessage(CHANNEL_ID, ctx.chat.id, mid);
    } catch (e) {
      console.error('Copy video failed:', e.message);
    }
  }
...
bot.start(async (ctx) => {
  resetSession(ctx.from.id);
  const s = getSession(ctx.from.id);
  // Start in Uzbek by default until language chosen
  s.lang = 'uz';
...
bot.on('callback_query', async (ctx) => {
  ...
  try {
    ...
    if (data.startsWith('cat_')) {
      const chosen = data.slice(4);
      // chosen text equals button label; verify exists
      const cats = CATEGORIES[s.lang];
      const real = cats.find((c) => c === chosen);
      if (!real) {
        await ctx.answerCbQuery('Invalid category', { show_alert: true });
        return;
      }
      s.data.category = real;
      await ctx.answerCbQuery(real);
...
bot.on('text', async (ctx) => {
  ...
  if (s.step === 'askPhone') {
    ...
    if (!normalized) {
      await ctx.reply(TEXT.invalidPhone[s.lang || 'uz']);
      return;
    }
...
bot.on(['video', 'video_note', 'document'], async (ctx) => {
  ...
  try {
    s.data.uploadedVideoIds.push(ctx.message.message_id);
    await ctx.reply('✅');
  } catch (e) {
    console.error('store video id failed', e);
  }
});
...
bot.on('photo', async (ctx) => {
  ...
  try {
    s.data.uploadedVideoIds.push(ctx.message.message_id); // will still forward
    await ctx.reply('✅');
  } catch (e) {}
});
const express = require("express");

// Bot va Express yaratamiz
const app = express();
...
// Render.com yoki Railway portni o‘zgaruvchi sifatida beradi
const PORT = process.env.PORT || 3000;

// Webhook o‘rnatish
bot.telegram.setWebhook(`https://your-app-name.onrender.com/webhook`);

app.listen(PORT, () => {
  console.log(`✅ Server is running on port ${PORT}`);
});
// ======== RUN ========
bot.launch().then(() => console.log('🤖 Hiring Bot is running...'));
